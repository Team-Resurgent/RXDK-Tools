...
{
    DIRL(const FIL &fil) : DIRF(fil)
    {
        m_fPrintingDir = TRUE;
    }

    FILR *PfilrCopy(void) const
    {
        return new DIRL(*this);
    }
...
void DIRL::PrintOne(void)
{
    ...
    printf("%s  %s %s\n", szTime, szSize, pdmfa->Name[0] ? pdmfa->Name : m_szName);
}
...
void DIRW::EndDir(void)
{
    ...
    *m_ppdirfNext = NULL;
    ...
    {
        rgpdirf[icol] = rgpdirf[icol - 1] ? rgpdirf[icol - 1]->m_pdirfNext : NULL;
    }
    crw = rgpdirf[0] ? 1 : 0;
    if (rgpdirf[ccol])
    {
