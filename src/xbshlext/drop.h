...
// CXboxDropTarget
//------------------------------------------------------------------------
class ATL_NO_VTABLE CXboxDropTarget : public CComObjectRootEx<CComMultiThreadModel>,
...
class CDropOperation
{
  public:
    CDropOperation(
        ...
        DWORD dwEffect,
        HWND hWnd) : m_pSelection(pSelection),
  ...
  private:
    ...
    //  For Async
    //
    static DWORD WINAPI ThreadProc(PVOID pvThis)
    {
        return reinterpret_cast<CDropOperation *>(pvThis)->TransferThread();
    }
