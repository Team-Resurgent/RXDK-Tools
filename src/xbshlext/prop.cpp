...
void CXboxPropertySheet::AddPage(
    CPropertyPage *pPage)
/*++
  Routine Description:
...
void CPropertyPage::FilloutPropSheetPage(
...
--*/
{
    ...
    if (hWndCtrl)
        SetWindowTextA(hWndCtrl, m_szDriveDescription);
...
INT_PTR CVolumeGeneralPage::OnDrawItem(UINT uItem, LPDRAWITEMSTRUCT pDrawItem)
{
    ...
    {
    case IDC_DRV_PIE: {
        DWORD dwPctX10 = m_ullTotalSpace ? (DWORD)((__int64)1000 * (m_ullTotalSpace - m_ullFreeSpace) / m_ullTotalSpace) : 1000;
...
CFSItemGeneralPage::CFSItemGeneralPage(
    ...
    UINT uItemCount) : m_pSelection(NULL), m_fNameChanged(false),
...
bool CFSItemGeneralPage::GetAttributesCallback(PVOID pvThis, XBOX_MULTIFILE_ATTRIBUTES *pMultiFileAttributes)
{
    ...
    if (IDD_FOLDERPROP == pThis->m_uTemplateResource)
        dwFolderCount--;
...
CConsoleGeneralPage::CConsoleGeneralPage(
    ...
    BOOL fMultipleConsoles) : m_pConnection(pConnection), m_fMultipleConsoles(fMultipleConsoles)
{
...
BOOL CConsoleGeneralPage::GetCaption(
    LPSTR szBuffer,
    ULONG ccBuffer)
{
...
void CConsoleGeneralPage::CompletePropSheetPage(
    PROPSHEETPAGEA *pPropSheetPage)
{
    //
    //  The client will have passed in m_fUseDriveOnTabName on construction.
...
CConsoleSecurityPage::CConsoleSecurityPage(
    IXboxConnection *pConnection, LPCSTR pszConsoleName) : m_pConnection(pConnection), m_dwAccess(0),
...
BOOL CConsoleSecurityPage::GetCaption(
    LPSTR szBuffer,
    ULONG ccBuffer)
{
    *szBuffer = '\0';
...
void CConsoleSecurityPage::CompletePropSheetPage(
    PROPSHEETPAGEA *pPropSheetPage)
{
...
CConsoleSettingsPage::CConsoleSettingsPage(
    IXboxConnection *pConnection) : m_pConnection(pConnection)
{
    _ASSERT(m_pConnection);
    m_pConnection->AddRef();
}

BOOL CConsoleSettingsPage::GetCaption(
    LPSTR szBuffer,
    ULONG ccBuffer)
{
    *szBuffer = '\0';
...
void CConsoleSettingsPage::CompletePropSheetPage(
    PROPSHEETPAGEA *pPropSheetPage)
{
...
CConsoleAdvancedPage ::CConsoleAdvancedPage(
    IXboxConnection *pConnection,
    LPCSTR pszConsoleName) : m_pConnection(pConnection)
{
    _ASSERT(m_pConnection);
    m_pConnection->AddRef();
    strcpy(m_szConsoleName, pszConsoleName);
...
BOOL CConsoleAdvancedPage::GetCaption(
    LPSTR szBuffer,
    ULONG ccBuffer)
{
    *szBuffer = '\0';
...
void CConsoleAdvancedPage::CompletePropSheetPage(
    PROPSHEETPAGEA *pPropSheetPage)
{
