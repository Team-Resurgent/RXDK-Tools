...
static struct
{
    ...
    {4361, 65535},      // RXDK rebuilt ship libs (wide dev approval)
    {VER_PRODUCTBUILD, 0},
    {0}};
                               ...
                               PXBEIMAGE_LIBRARY_VERSION rglibv, int clibv, PLIB_ERROR_FUNCTION pfnErr)
{
    ...
    for (; clibv--; ++plibv)
    {
        ...
        if (i < 0)
        {
            if (iverXapi >= 0 && rgver[iverXapi].wMinVer == plibv->BuildVersion && plibv->QFEVersion == 1)
                i = 2;
        ...
        if (i > 0)
        {
            for (idep = 0; rgdep[idep].szDependentName; ++idep)
            {
                    ...
                    plibv->BuildVersion <= rgdep[idep].wDependentNotNewerThan)
                {
                    ...
                    {
                        if (plibvSup != plibv && 0 == memcmp(plibv->LibraryName, rgdep[idep].szSupportingName, XBEIMAGE_LIBRARY_VERSION_NAME_LENGTH) &&
