...
HRESULT CWizard::DoWizard(HWND hwndParent)
{
    ...
    propSheetHeader.hInstance = g_app.hInstance;
...
void CWizardPage::FilloutPropSheetPage(PROPSHEETPAGEA *pPropSheetPage, HFONT hTitleFont)
{
    ...
    pPropSheetPage->hInstance = g_app.hInstance;
...
INT_PTR CAddConsoleMakeDefault::OnSetActive()
{
    ...
    char szQuestion[120];
    m_pAddConsoleData->GetConsoleName(szConsoleName);
    WindowUtils::rsprintf(szQuestion, ARRAYSIZE(szQuestion), IDS_ADDCONSOLE_MAKEDEFAULT_QUESTION, szConsoleName);
...
INT_PTR CAddConsoleFinish::OnSetActive()
{
    ...
    //
    LoadStringA(g_app.hInstance, m_pAddConsoleData->GetMakeDefault() ? IDS_LITERAL_YES : IDS_LITERAL_NO, szTemp, sizeof(szTemp));
    ...
    if (m_pAddConsoleData->ShowAccessDenied())
    {
        ...
        if (dwAccess & DMPL_PRIV_READ)
        {
            ...
            LoadStringA(g_app.hInstance, IDS_PERMISSION_READ, szTemp, sizeof(szTemp));
            strcat(szPermissions, szTemp);
            if (dwAccess)
                strcat(szPermissions, ", ");
        }
        if (dwAccess & DMPL_PRIV_WRITE)
        {
            dwAccess &= ~DMPL_PRIV_WRITE;
            LoadStringA(g_app.hInstance, IDS_PERMISSION_WRITE, szTemp, sizeof(szTemp));
            strcat(szPermissions, szTemp);
            if (dwAccess)
                strcat(szPermissions, ", ");
        }
        if (dwAccess & DMPL_PRIV_CONFIGURE)
        {
            ...
            LoadStringA(g_app.hInstance, IDS_PERMISSION_CONFIGURE, szTemp, sizeof(szTemp));
            strcat(szPermissions, szTemp);
            if (dwAccess)
                strcat(szPermissions, ", ");
        }
        if (dwAccess & DMPL_PRIV_CONTROL)
        {
            ...
            LoadStringA(g_app.hInstance, IDS_PERMISSION_CONTROL, szTemp, sizeof(szTemp));
            strcat(szPermissions, szTemp);
            if (dwAccess)
                strcat(szPermissions, ", ");
        }
        if (dwAccess & DMPL_PRIV_MANAGE)
        {
            ...
            LoadStringA(g_app.hInstance, IDS_PERMISSION_MANAGE, szTemp, sizeof(szTemp));
            strcat(szPermissions, szTemp);
        }
...
HRESULT CAddConsoleData::OnFinish()
{
    ...
    if (m_dwAccess != m_dwDesiredAccess)
    {
        ...
        hr = GetConsoleConnection(m_szConsoleName, &pConnection);
        if (SUCCEEDED(hr))
        {
...
BOOL CAddConsoleData::SetConsoleName(LPCSTR pszConsoleName)
{
    ...
    if (SUCCEEDED(GetConsoleConnection(m_szConsoleName, &pConnection)))
    {
...
void ExecuteAddConsoleWizard(void)
{
    HRESULT hr;
    ...
    wizard.DoWizard(g_app.hwndMain);
    AppRefreshTree();
}
