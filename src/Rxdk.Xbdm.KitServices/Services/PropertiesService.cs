...
public class PropertySession : IDisposable
{
...
public class PropertiesService
{
    ...
    private static PropertyContext BuildDriveContext(XbdmConnection conn, PropertyRequest request)
    {
        ...
        var description = FormattingHelper.GetVolumeDisplayName(driveLetter);
        ...
        return new PropertyContext
        {
            ...
            Drive = new DriveGeneralInfo
            {
                ...
                DriveType = FormattingHelper.GetDriveTypeName(driveLetter),
                TotalBytes = total,
                FreeBytes = free,
            },
        };
    ...
    public void ApplyFileAttributes(XbdmConnection conn, FileGeneralInfo file, bool recursive = false)
    {
        ...
        foreach (var item in GetAttributeTargets(conn, file, recursive))
        {
            var entry = conn.GetFileAttributes(item.WirePath);
    ...
    public static bool ShouldConfirmRecursiveAttributes(PropertyTargetKind kind, FileGeneralInfo file)
    {
        if (kind == PropertyTargetKind.Folder)
            return file.FileCount > 0 || file.FolderCount > 0;

        if (kind == PropertyTargetKind.MultiFile)
            return file.Items.Any(static item => item.IsDirectory);

        return false;
    }

    public static string BuildAttributeChangeSummary(bool setReadOnly, bool clearReadOnly, bool setHidden, bool clearHidden)
    {
        var parts = new List<string>();
        if (setReadOnly)
            parts.Add("set read-only");
        if (clearReadOnly)
            parts.Add("unset read-only");
        if (setHidden)
            parts.Add("hide");
        if (clearHidden)
            parts.Add("unhide");
    ...
    private static IEnumerable<FileSelectionItem> GetAttributeTargets(
        XbdmConnection conn,
        FileGeneralInfo file,
        bool recursive)
    {
        foreach (var item in file.Items)
        {
            ...
            {
                foreach (var child in EnumerateDirectoryItems(conn, item.WirePath))
                    yield return child;
            }
        }
    }

    private static IEnumerable<FileSelectionItem> EnumerateDirectoryItems(XbdmConnection conn, string wirePath)
    {
        foreach (var entry in conn.ListDirectory(wirePath))
        {
            var childWire = $"{wirePath.TrimEnd('\\')}\\{entry.Name}";
            var isDirectory = (entry.Attributes & XbdmConstants.AttrDirectory) != 0;
            yield return new FileSelectionItem
            {
                Name = entry.Name,
                WirePath = childWire,
            ...
            if (isDirectory)
            {
                foreach (var nested in EnumerateDirectoryItems(conn, childWire))
                    yield return nested;
            }
        }
    }
    ...
    public static PropertyRequest? FromSelection(string? console, string? currentPath, IReadOnlyList<FileSelectionItem> items)
    {
        ...
        if (items.Count == 1)
        {
            var itemPath = string.IsNullOrWhiteSpace(currentPath)
                ? items[0].Name
                : WirePathService.GetItemDisplayPath(currentPath, items[0].Name);

            if (WirePathService.IsVolumeDisplayPath(itemPath))
            {
                var letter = WirePathService.GetVolumeLetterFromDisplayPath(itemPath);
                return new PropertyRequest
                {
                    Kind = PropertyTargetKind.Drive,
                    ConsoleName = console,
            ...
            if (WirePathService.IsConsoleDisplayPath(itemPath))
            {
                return new PropertyRequest
                {
                    Kind = PropertyTargetKind.Console,
