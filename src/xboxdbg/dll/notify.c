    ...
    for (ich = 0; ich < cchCmd; ++ich)
    {
        if (sz[ich] == '!')
        {
            ...
            for (psess = psessFirst; psess; psess = psess->psessNext)
            {
                ...
                {
                    if (psess->rgpenh[ienh] && !_strnicmp(sz, psess->rgpenh[ienh]->szExtName, ich) &&
...
HRESULT HrMakePersistent()
{
    ...
    if (f)
    {
        sprintf(sz, "NOTIFYAT PORT=%d%s", usPersistPort, dwPersistFlags & DM_DEBUGSESSION ? " debug" : "");
...
HRESULT HrEstablishNotificationConnection()
{
    ...
    for (;;)
    {
        ...
        {
            n = WaitForMultipleObjects(2, rgh, FALSE, 60000 - (tickNow - tickStart));
            if (n == WAIT_OBJECT_0)
            {
        ...
        while (pdconNotify && pdconNotify->s != INVALID_SOCKET)
        {
            ...
            case WAIT_OBJECT_0:
                if (pdconNotify->ibBuf >= pdconNotify->cbBuf)
                {
                    int cb;
                    /* Data to receive */
                    cb = recv(pdconNotify->s, pdconNotify->rgbBuf, sizeof pdconNotify->rgbBuf - 1, 0);
                    if (cb <= 0)
                    {
