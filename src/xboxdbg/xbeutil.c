                              ...
                              DWORD cchPath)
{
    ...
    for (;;)
    {
        ...
        if (INVALID_FILE_SIZE == SetFilePointer(hFile, XbeVaOffset(xih.DebugFileName, xih.BaseAddress), NULL, FILE_BEGIN))
            break;
        ...
        if (INVALID_FILE_SIZE == SetFilePointer(hFile, XbeVaOffset(xih.DebugPathName, xih.BaseAddress), NULL, FILE_BEGIN))
            break;
...
HRESULT XbeGetBuilderArgs(LPCSTR szName, LPSTR szArgs, LPDWORD pcchArgs)
{
    ...
    else if (INVALID_FILE_SIZE == SetFilePointer(h, XbeVaOffset(xih.Certificate, xih.BaseAddress), NULL, FILE_BEGIN) ||
             !ReadFile(h, &xbc, sizeof xbc, &cb, NULL) || cb < sizeof xbc)
        hr = XBDM_FILEERROR;
    else if (INVALID_FILE_SIZE == SetFilePointer(h, XbeVaOffset(xih.DebugPathName, xih.BaseAddress), NULL, FILE_BEGIN))
        hr = XBDM_FILEERROR;
    else
    {
        char *pch;
