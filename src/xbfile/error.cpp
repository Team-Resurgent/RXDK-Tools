               ...
               LPCTSTR szFailSafe, va_list val)
{
    ...
    if (!LoadString(GetModuleHandle(NULL), ids, szRes, sizeof szRes / sizeof(TCHAR)))
        ids = 0;
    if (ids == 0)
    {
        if (fFailSafe)
...
void ShowError(HRESULT hr, DWORD ids, DWORD idsPrefix, va_list val)
{
    ...
    if (!LoadString(GetModuleHandle(NULL), idsPrefix, szError, sizeof szError / sizeof(TCHAR)))
        szError[0] = 0;
